...
export function TopicFollowPage() {
  const navigate = useNavigate();
  const { topics } = useStore();
  const followedTopics = topics.slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 pb-20">
      <div className="bg-slate-800/80 backdrop-blur-sm border-b border-slate-700/50 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <h1 className="font-semibold text-lg text-slate-100">我的关注</h1>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="px-2 py-0.5 bg-slate-700 text-slate-400 text-xs rounded-full">{topic.debate_type}</span>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-slate-500 text-xs">
                    <span className="flex items-center gap-1"><Users size={12} /> {total} 参与</span>
                  </div>
                </div>
              </div>
            </motion.div>
